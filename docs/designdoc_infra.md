...
| **Database** | Cloud SQL Instance (PostgreSQL) | Primary instance, network configuration (private IP), and initial database/user setup. |
...
### Cost and Security:
* **Database:** Cloud SQL is provisioned to automatically scale storage, ensuring high availability (HA) configuration for production. Access is via private IP; Cloud Run uses a VPC connector.
* **IAM:** Principle of least privilege is enforced. Service Accounts are granted only the minimum required permissions to execute their specific tasks (e.g., the Go backend can only read/write to its own DB/Storage paths). Signed URLs are short-lived for Storage access; no public buckets.
